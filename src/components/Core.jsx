        ...
        <div className="flex flex-wrap">
          {loading ? (
            ...
            cardsData.map((card, index) => (
              <div
                key={index}
                className="flex-none w-full sm:w-1/2 md:w-1/3 lg:w-1/6 xl:w-1/6 p-2"
              >
                <Link to={`/core/${card.id}`} className="block" style={{ textDecoration: 'none', color: 'inherit' }}>
                  <div className="shadow-lg overflow-hidden rounded-lg transition-transform transform hover:scale-105 h-full flex flex-col">
                    <img
